...
{
  if (P_sigma.isZero() and P.isZero()) // missed P even requirement without
    // second condition
    return true;
  if (P.isZero() and not(P_sigma.isZero()))
      return false; // now P isn't zero
  if (not(P_sigma.isZero()) and P_sigma.degree()>P.degree())
    return false;
